...
| 2026-04-29 | **§7.2 revision — duration soft flags replace hard drops** (Harvey/Porter/AIDI DQ signal synthesis from 50+ CALBEAF-141 runs). Hard `duration_violation` drop replaced by: (a) Case A re-assignment — SHORT+≥24h → Workshop; LONG+<24h+SHORT keyword → best SHORT; LONG+<24h no SHORT keyword → keep LONG + flag; (b) Case B disambiguation — both LONG+SHORT keywords + duration<24h → SHORT wins; (c) per-category ceilings — Festival 240h / Marathon 120h / Encuentro 96h / Workshop 72h (soft flag, event still loads). §12.2 quality_flag reasons updated (`duration_violation` → `duration_reassigned` + `duration_ceiling_exceeded`). slc-wasatch delta: 9 hard-drops → 0; +11 classify_loadable; 153 events written to playground. Code: commits b611c8b + 85fe94a. AIDI sign-off: 2026-04-29. | Harvey 2026-04-29; AIDI overseer pass |
| 2026-04-30 | **First --live to TT_Test appId=99 — 5 loader bugs fixed** (commits 6783e91 + cc881c1 + 53bc34a). Surfaced when actual TT_Test writes started; playground/dry-run paths had hidden them. (1) Venue POST 201 response shape — BE returns venue doc directly with `_id`, not wrapped `{venue:...}`; tolerant union type. (2) Same fix on 409→GET path. (3) Organizer shortName retry — cap 5→20 + random hex suffix from attempt 2 (sequential -1/-2/-3/-4 collided through cap on `argentinetango4u`). (4) `fetchWithRetry` on all BE calls — 3 attempts, 1s/3s/9s exponential backoff, handles BE-AF transient 5xx (Mongo timeouts on its end). (5) **CRITICAL: `EventDoc.startDate` typed as Date not String** — was string-typed; broke `$gte/$lt` Mongo dedup query → would have created 901+ dup groups. `toUtcDate()` forces Z-suffix on iCal naive ISO ("2025-06-04T19:45:00") to honor Harvey's UTC-Z end-to-end contract. Final --live run: 1,943 attempted / 1,914 inserted / 29 skipped (dedup hit) / 0 failed / 0 verified dup groups. State remains `locked`; this is contract-as-tracked-by-changelog. | Narvest first --live shakeout 2026-04-30 |
| 2026-04-30 | **§8.3 dedup metadata — BE-AF strips unknown fields from venue + organizer POSTs** (verified 2026-04-30 first --live). `nh_batch_id`, `isDiscovered`, `discoverySource` are stripped from venue/organizer POST request bodies; only BE-known schema fields survive. Events go via direct-Mongo `insertOne` so they keep all fields. **Implication:** rollback for venues/organizers cannot use `nh_batch_id`; current --live runs use `appId: 99` sentinel as the safe discriminator. **For Phase 4+5 cutover to appId=1, three options open:** (a) Fulton allows `nh_batch_id` on venue/organizer POST endpoints (cleanest), (b) niche-harvest writes venues/organizers direct-Mongo too (mirrors Porter's pattern; bypasses BE-AF for those endpoints; changes §8.2), (c) dedup purely on natural keys without provenance flag. AIDI + Fulton ping pending. | Narvest discovery 2026-04-30 |
| 2026-05-01 | **CALBEAF-168 + field rename `nh_batch_id` → `discoveryBatchId`** (Fulton 2026-05-01 — Option 1 chosen, generalized field name to avoid per-consumer field sprawl). Three explicitly-modeled optional fields will be added to BE-AF Venue + Organizer schemas: (1) `isDiscovered: Boolean` — extends from events to venues + organizers for consistency, (2) `discoverySource: String` — free-form provenance label, same lane as existing event field (current PROD values: "fb-conditioner", "harvester", "porter", "porter-booker", "porter-harvey"; "niche-harvest" slots in as a peer; no enum validation), (3) `discoveryBatchId: String` — per-cycle batch UUID for rollback/lineage. Stored verbatim, no behavior beyond type. niche-harvest renamed `nh_batch_id` → `discoveryBatchId` across loader/denorm/interface/dashboard/tests (33 refs) on commit forthcoming. Endpoint patch lands TEST 2026-05-02 AM; 50-event smoke verifies all 3 fields land on venue + organizer rows; PROD bundles with Fulton's next deploy window. | Narvest + Fulton CALBEAF-168 2026-05-01 |
